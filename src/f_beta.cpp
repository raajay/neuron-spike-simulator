...
void beta::initBeta(parameters param)
{
	num = param.neuronNum;
	...
	for(int i=0;i<num;i++)
	{
		for(int j=0;j<num;j++)
		{
			...
			tempPkt.delays.push_back(myRandInt(param.delayRandL,param.delayRandH));
		...
		for(int i=0;i<num;i++)
		{
			for(int j=0;j<num;j++)
			{
				for(int k=j+1;k<num;k++)
				{
					...
					temp.delays.push_back(myRandInt(param.delayRandL,param.delayRandH));
					temp.delays.push_back(myRandInt(param.delayRandL,param.delayRandH));
					if(myRand(0,1) < param.pConnect)
					{
...
#endif
void beta::loadEpisodeProbabilities(parameters param)
{
	fstream fin;
	fin.open(param.episodeFile.c_str(),ios::in);
	if(fin == NULL) { errAbort(" Episode File not found. Exiting... ");}
	int count;
	fin >> count;
	for(int i=0;i < count;i++)
	{
		...
		if(order == 2)
		{
			if(episodePkt.parents.size() != 1)  {cout << "Episode number -" << i << " is wrong. "<<endl; exit(0);}
			betas[0][child-1][index] = episodePkt;

		}
		else if(order == 3)
		{
			if(episodePkt.parents.size() != 2)  {cout << "Episode number -" << i << " is wrong. "<<endl; exit(0);}
			if(param.flag3Rand == true)
		...
		{
			if(episodePkt.parents.size() != order-1)  {cout << "Episode number -" << i << " is wrong. "<<endl; exit(0);}
...
void beta::displayBeta(int order)
{
...
void beta::calculateBetas(parameters param)
{
	...
	for(int i =0 ;i < betas[0].size(); i++)
	{
		for(int j=0; j < betas[0][i].size(); j++)
		{
			...
			betas[0][i][j].beta = -param.theta - log(param.lambdaM / lambdaTemp -1);
		}
	}
	...
	for(int i=0;maxOrder >= 3 && i < betas[1].size(); i++)
	{
		for(int j = 0 ; j < betas[1][i].size(); j++)
		{
			...
			for(int k = 0;k < betas[1][i][j].parents.size();k++)
			{
				...
				if(betas[0][i][parent-1].delays[0] == betas[1][i][j].delays[k])
					sigmaBetaSub = sigmaBetaSub + betas[0][i][parent-1].beta;
			}
			betas[1][i][j].beta = -param.theta - sigmaBetaSub - log(param.lambdaM / lambdaTemp -1);
		}
	}
#if 1
	...
	for(int k = 2 ; k < maxOrder-1; k++)
	{
		for(int i=0;i< betas[k].size();i++)
		{
			for(int j = 0; j < betas[k][i].size() ;j++)
			{
				...
				{
					lambdaTemp = sqrt(2*betas[k][i][j].prob)/param.teeUpdate;
				}
				//lambdaTemp = - log(1 - betas[k][i][j].prob) / param.teeUpdate;
				...
				for(int m = 0;m < betas[k][i][j].parents.size();m++)
				{
					...
					if(betas[k][i][j].delays[m] == betas[0][i][parent-1].delays[0])
						sigmaBetaSub = sigmaBetaSub + betas[0][i][parent-1].beta;
				}
				...
				betas[k][i][j].beta = -param.theta - sigmaBetaSub - log(param.lambdaM / lambdaTemp -1 );
			}
		}
	}
#endif
...
void beta::displayMiniPkt(int event,vector<miniPkt> vec)
{
	cout << event+1<<" -> { ";
...
void beta::displayMiniPkt(int event,list<miniPkt> vec)
{
	cout << event+1<<" -> { ";
...
double beta::getVoltage(int event,lookBack buf)
{
	int n = event-1;
	double voltage = 0.0;
	for(int order = 0 ; order < betas.size() ; order++)
	{
		...
		{
			if(betas[order][n][i].isConnection == false) continue;
			bool flag = true;
			for(int k=0;k < order+1 && flag == true;k++)
			{
				if(buf.occurence(betas[order][n][i].delays[k],betas[order][n][i].parents[k]) == false )
					flag = false;
			}
			if(flag == true)
...
void beta::get2Values(vector< vector<data2> > &outBeta)
{
	for(int i=0;i<num;i++)
	{
		for(int j=0;j<num;j++)
...
void beta::simulation_fast(parameters param)
{
			...
			for(int j=0;j<betas[odr][i].size();j++)
			{
				if(betas[odr][i][j].isConnection == true)
				{
					for(int k=0;k<betas[odr][i][j].parents.size();k++)
					{
						...
						bool flag = true;int pos;
						for(int chk=0;chk<frontConnect[parent-1].size();chk++)
						{
							if(frontConnect[parent-1][chk].event == i)
							{
								pos = chk;flag = false; break;
							}
						}
						miniPkt temp;
						temp.event = i;
						temp.time = delay;
						if(flag==true) frontConnect[parent-1].push_back(temp);
						else frontConnect[parent-1][pos] = temp;
					}
				}
			}
		}
			...
			for(int odr=1;odr<param.order-1;odr++)
			{
				for(int pkts=0;pkts<betas[odr][j].size();pkts++)
				{
					...
					for(int k = 0;addVoltage==true && k<betas[odr][j][pkts].parents.size();k++)
					{
						...
						{
							if(i*param.teeUpdate==(*listIter).time && (*listIter).event == parent-1)
							{

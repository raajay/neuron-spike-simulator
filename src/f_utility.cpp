...
double
myRand (double low, double high)
{
  if (low > high)
    errEcho ("in myRand : Interval not valid");
...
int
myRandInt (int low, int high)
{
  if (low > high)
    errEcho ("in myRandInt : Interval not valid");
  double temp = myRand (low, high);
  return int (temp);
}

double
getExpSpikeTime (double lambdaM, double voltage)
{
...
double
getGamma2SpikeTime (double lambdaM, double voltage)
{
  return (getExpSpikeTime (lambdaM, voltage) +
	  getExpSpikeTime (lambdaM, voltage));
}

void
mySort (vector < miniPkt > &events)
{
  for (int x = 0; x < events.size (); x++)
      ...
      for (int y = x + 1; y < events.size (); y++)
	{
	  ...
	  if (events[x].time > events[y].time)
	    {
	      timetemp = events[y].time;
	      eventtemp = events[y].event;
	      events[y].time = events[x].time;
	      events[y].event = events[x].event;
	      events[x].time = timetemp;
	      events[x].event = eventtemp;
	    }
	}
    }
}
...
bool
isContained (packet sub, packet set)
{
  if (sub.parents.size () >= set.parents.size ())
    return false;
  bool flag1 = true;
  for (int i = 0; i < sub.parents.size () && (flag1 == true); i++)
	...
	{
	  if (sub.parents[i] == set.parents[j]
	      && sub.delays[i] == set.delays[j])
	    {
	      flag2 = true;
	    }
	}
...
#if 0
void
simulation (beta Beta, lookBack & buffer, parameters param)
{
  fstream out;
...
#if 0
void
beta::simulation_spl_order2 (parameters param)
{
		  ...
		  for (int k = 0; k < betas[odr][i][j].parents.size (); k++)
		    {
		      ...
		      bool flag = true;
		      for (int chk = 0; chk < frontConnect[parent].size ();
			   chk++)
			{
...
// FOR DEBUGGING
  ...
  for (int i = 0; i < frontConnect.size (); i++)
	...
	{
	  cout << frontConnect[i][j].event +
	    1 << "(" << frontConnect[i][j].time << ") ";
	}
      cout << " }" << endl;
    }
#endif

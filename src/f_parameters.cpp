...
void parameters::getParameters(string ipFile)
{
	fstream in;
    ...
    outfile = "stream.txt";
		...
		if(str =="numberOfNeurons:")
				{in >> neuronNum;cout << "Number of Neurons = "<<neuronNum<<endl;}
		else if(str =="pRandLow:")
				{in >> pRandL;cout << "Lower limit of Random Probability = "<<pRandL<<endl;}
		else if(str == "pRandHigh:")
				{in >> pRandH;cout << "Upper limit of Random Probability = "<<pRandH<<endl;}
		else if(str == "delayRandLow:")
				{in >> delayRandL;cout << "Lower Limit of Random Delay = " << delayRandL << endl; }
		else if(str == "delayRandHigh:")
				{in >> delayRandH;cout << "Upper Limit of Random Delay = " << delayRandH << endl; }
		else if(str == "randomFrequency:")
				{in >> randFreq;cout << "Random firing rate = " << randFreq << " Hz."<<endl;}
		else if(str == "tUpdate:")
				{in >> teeUpdate;cout << "Time step  = " << teeUpdate << " seconds."<<endl;}
		else if(str == "simulationTime:")
				{in >> teeSim;cout << "Total Simulation for " << teeSim << " seconds."<<endl;}
		//else if(str == "stimulus_file:")
		//		{in >> stimulusFile;cout << "Stimulus taken from " << stimulusFile <<endl;}
		else if(str == "maxOrderOfInteraction:")
				{in >> order;cout << "Maximum order of interacton is : " << order <<endl;}
		else if(str == "percentageConnections:")
                {in >> pConnect; cout << "Percentage of random connections = " << pConnect << endl;}
		else if(str == "spikeDistribution:")
		{
			string temp; in >> temp;
			if(temp == "gamma2") dist_flag = 1;
			else dist_flag = 0;
...
bool lookBack::occurence(int delay,int event)
{
	//if(table.size() < delay) return false;
	bool falg = false;
	for(int i = 0; i< table[delay-1].size() && falg == false ;i++)
	{
		if(table[delay-1][i] == event) falg = true;
	}
	return falg;
}

void lookBack::updateLookBack(vector<int> newVect)
{
	int siz = table.size();
	table[siz-1].clear();
	table.pop_back();
...
void lookBack::dumpStream(string outfile)
{
	fstream out;
